...
void MoveController(int** matrix, int padding, int M, int N, int tac_toe, int& row, int& col) {
    ...
    for (int i = padding / 2; i < M + padding * 2 - padding / 2; i++)
    {
        for (int j = padding / 2; j < N + padding * 2 - padding / 2; j++)
        {
            if (matrix[i][j] == 0)
            {
                ...
                col = j - padding / 2;
                matrix[i][j] = tac_toe;
                if (fiveInRow(matrix, padding, 2, i, j, fourCounter, rowN, colN))
                {
                    //matrix[i][j] = 9;
    ...
    for (int i = padding / 2; i < M + padding * 2 - padding / 2; i++)
    {
        for (int j = padding / 2; j < N + padding * 2 - padding / 2; j++)
        {
            if (matrix[i][j] == 0)
            {
                ...
                col = j - padding / 2;
                matrix[i][j] = tac_toe;
                fiveInRow(matrix, padding, 1, i, j, fourCounter, rowN, colN);
                matrix[i][j] = 0;
            }
        }
    }
    ...
    if (fourCounter > 1) {
        cout << "Вы не можете победить!" << endl;
        row = rowN;
        col = colN;
        return;
    }
...
bool canIWin(int** matrix, int tac_toe, int row, int col, int shift_row, int shift_col, int& fourCounter, int &rowN, int &colN)
{
...
bool fiveInRow(int** matrix, int padding, int tac_toe, int row, int col, int& fourCounter, int& rowN, int& colN) {
    //Block Горизонталь
    if (canIWin(matrix, tac_toe, row, col, 0, 1, fourCounter, rowN, colN))
    {   
        return true;
    }

    //Block Вертикаль
    if (canIWin(matrix, tac_toe, row, col, 1, 0, fourCounter, rowN, colN))
    {
        return true;
    }

    //Block Главная диагональ
    if (canIWin(matrix, tac_toe, row, col, 1, 1, fourCounter, rowN, colN))
    {
        return true;
    }
...
bool isClosedCastRow(int** matrix, int row, int col, int shift_row, int shift_col) {
...
bool checkQuad(int** matrix, int row, int col, int shift_row, int shift_col, int& fourCounter)
{
    ...
    while (matrix[d_row][d_col] == 2)
    {
        ...
        if (counter_of_figures == 4) {
            if (!isClosedCastRow(matrix, row, col, shift_row, shift_col)) {
                return true;
            }
        }
        d_col += shift_col;
    ...
    if (isClosedRow(matrix, row, col, shift_row, shift_col) and counterLeftF == 0 and counterRightF == 0 and counter_of_figures > 4)
    {
         return true;
